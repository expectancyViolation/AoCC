...
void print_hand(const struct hand *h) {
    printf("hand:\n\t%d\n\t%d\n\t%s\n\t%ld\n", h->largest_match, h->second_largest_match, h->unsorted_hand, h->bid);
}
...
int compare_hand(const struct hand *h1, const struct hand *h2) {
    ...
    const int second_largest_match_diff = CMP(h1->second_largest_match,h2->second_largest_match);
    if(second_largest_match_diff!=0)
        return second_largest_match_diff;
    for(int i=0;i<5;i++){
        const int curr_card_compare= compare_card_strength(h1->unsorted_hand+i,h2->unsorted_hand+i);
...
int compare_hand_reverse(const struct hand *h1, const struct hand *h2) {
   return compare_hand(h2,h1);
}
...
void parse_hand(char *line, struct hand *out) {
    ...
    // check matches
    qsort(sorted_hand, HAND_SIZE, sizeof(char), (__compar_fn_t) compare_card_strength);
...
struct two_part_result *day07(char *buf, long buf_len) {
    ...
    qsort(hands, cvector_size(hands), sizeof (*hands), (__compar_fn_t) compare_hand);
